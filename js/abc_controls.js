...
function stopABC(song) {
  abcStopped = 1
  inst.silence();
};

function abcSliderChanged(value, song){
    stopABC(song);
    setABCSpeed(value, song);
};

function setABCSpeed(value, song){
    playingNow = 1;
    abcStopped = 0;
    inst.silence();
    inst.play({ tempo:value },song.value, function(){playingNow = 0;loopABCTune(value, song);});
};

function loopABCTune(value, song){
    inst.silence();
